<div class="bg-gray-900 p-4 h-screen w-screen flex flex-col justify-center my-auto">
    ...
    <h1 class="text-white text-3xl my-4 text-center font-bold">Is Make a Video Call For You</h1>
    <x-splade-data default="{calling: false}">
        <div v-if="data.calling" class="flex flex-col justify-center mx-auto text-white text-center">
            ...
            <h1 class="text-center text-white text-3xl text-green-500">Connection....</h1>
        </div>
        <div v-else class="flex justify-center my-4">
            <div class="flex flex-col justify-center text-center text-white">
                <Link @click="data.calling = true" href="{{route(config('tomato-chat.routes.name') .  'chat.join', [$user->id, 'video'])}}"  type="submit"  class="h-12 w-12 bg-green-500 text-center text-white rounded-full mx-4  my-2 p-2">
                    <i class="bx bx-sm bx-check mt-1"></i>
                </Link>
                <h1>Accept</h1>
            </div>
            <div class="flex flex-col justify-center text-center text-white">
                <Link href="{{url()->previous()}}" class="h-12 w-12 bg-red-500 text-center text-white rounded-full mx-4 my-2 p-2">
                <i class="bx bx-sm bx-x mt-1"></i>
                </Link>
                <h1>Cancel</h1>
            </div>
